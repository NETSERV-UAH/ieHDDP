...
set_field(struct packet *pkt, struct ofl_action_set_field *act )
{
    ...
    if (pkt->handle_std->valid)
    {
        ...
        switch(act->field->header){
            ...
            case OXM_OF_EHDDP_FLAGS :
            {
                memcpy(&pkt->handle_std->proto->ehddp->flags,act->field->value, sizeof(uint8_t));
                break;
            }
            case OXM_OF_EHDDP_OPCODE :
            {
                memcpy(&pkt->handle_std->proto->ehddp->opcode,act->field->value, sizeof(uint8_t));
                break;
            }
            case OXM_OF_EHDDP_NUM_DEVICE:
            {
                memcpy(&pkt->handle_std->proto->ehddp->num_devices,act->field->value, sizeof(uint8_t));
                break;
            }
            case OXM_OF_EHDDP_NUM_SEC:
            {
                memcpy(&pkt->handle_std->proto->ehddp->num_sec,act->field->value, sizeof(uint64_t));
                break;
            }
            case OXM_OF_EHDDP_PRE_MAC_SIZ:
            {
                memcpy(&pkt->handle_std->proto->ehddp->nxt_mac,act->field->value, sizeof(uint64_t));
                break;
            }
            case OXM_OF_EHDDP_PRE_MAC:
            {
                memcpy(&pkt->handle_std->proto->ehddp->previous_size_mac,act->field->value, sizeof(uint8_t));
                break;
            }
            case OXM_OF_EHDDP_NUM_ACK:
            {
                memcpy(&pkt->handle_std->proto->ehddp->num_ack,act->field->value, sizeof(uint64_t));
                break;
            }
            case OXM_OF_EHDDP_LAS_MAC:
            {
                memcpy(&pkt->handle_std->proto->ehddp->last_mac,act->field->value, sizeof(uint64_t));
                break;
            }
            case OXM_OF_EHDDP_SRC_MAC:
            {
                memcpy(&pkt->handle_std->proto->ehddp->src_mac,act->field->value, sizeof(uint64_t));
                break;
            }
            case OXM_OF_EHDDP_TIM_BLO:
            {
                memcpy(&pkt->handle_std->proto->ehddp->time_block,act->field->value, sizeof(uint32_t));
                break;
            }
            case OXM_OF_EHDDP_CONFIG:
            case OXM_OF_EHDDP_TYPE_DEVICE:
            case OXM_OF_EHDDP_IDS:
            case OXM_OF_EHDDP_OUT_PORTS:
            case OXM_OF_EHDDP_IN_PORTS:
            {
                break;
            }
...
size_t size_data_to_read(uint8_t config, int type_read){
    ...
    switch (type_read)
    {
        case 1: /* Type device case */
            size = ((config | 0b11000000) << 6) + 1;
            break;
        
        case 2: /* ID device case */
            size = ((config | 0b00111000) << 3) + 1;
            break;

        case 3: /* Port case */
            size = ((config | 0b00000110) << 1) + 1;
            break;
        default:
            size = 1;
            break;
