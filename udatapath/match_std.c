...
static inline bool match_port(uint8_t *a, uint8_t *b)
{
    uint32_t * a1=(uint32_t *) a;
    uint32_t * b1=(uint32_t *) b;
    return(memcmp(a1,b1,sizeof(uint32_t)*EHDDP_MAX_ELEMENTS)==0);
}

static inline bool match_macs(uint8_t *a, uint8_t *b)
{
    uint64_t * a1=(uint64_t *) a;
    uint64_t * b1=(uint64_t *) b;
    return(memcmp(a1,b1,sizeof(uint64_t)*EHDDP_MAX_ELEMENTS)==0);
}
/*Modificacion UAH Discovery hybrid topologies, JAH-*/
    ...
    HMAP_FOR_EACH(f, struct ofl_match_tlv, hmap_node, &flow_match->match_fields)
    {
        ...
        switch (field_len) {
            ...
            case 16:
                ...
                break;
            /*Modificacion UAH Discovery hybrid topologies, JAH-*/   
            case (sizeof(uint32_t)*EHDDP_MAX_ELEMENTS):
                if (!match_port(flow_val,packet_val))
                    return false;
                break;
            case (sizeof(uint64_t)*EHDDP_MAX_ELEMENTS):
                if (!match_macs(flow_val,packet_val))
                    return false;
                break;
            /*Modificacion UAH Discovery hybrid topologies, JAH-*/
            default:
...
match_std_strict(struct ofl_match *a, struct ofl_match *b) {
    ...
    HMAP_FOR_EACH(flow_mod_match, struct ofl_match_tlv, hmap_node, &a->match_fields)
    {
        ...
        switch (field_len) {
            ...
            /*Modificacion UAH Discovery hybrid topologies, JAH-*/   
            case (sizeof(uint32_t)*EHDDP_MAX_ELEMENTS):
                if (!match_port(flow_mod_val,flow_entry_val))
                    return false;
                break;
            case (sizeof(uint64_t)*EHDDP_MAX_ELEMENTS):
                if (!match_macs(flow_mod_val,flow_entry_val))
                    return false;
                break;
            /*Modificacion UAH Discovery hybrid topologies, JAH-*/
            default:
