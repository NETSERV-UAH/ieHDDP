...
/**
 * @brief Provider de dispositivos para eHDDP
 * @author Joaquin Alvarez Horcajo
 */


public class eHDDPproviderdevices implements DeviceProvider {
    ...
    public eHDDPproviderdevices(ProviderId pid){
        PID = pid;
    }
     ...
     * @param Packet_eHDDP eHDDPpacket
    ...
    public boolean checkdevices(DeviceProviderService dpsv, DeviceService dsv, DeviceProvider dpr,
                                eHDDPpacket Packet_eHDDP, String TYPE_SENSORS[], Set<String> configureNodes,
                                int num_dev_sdn_file){
        short type_device[] = Packet_eHDDP.getTypedevices();
        String Mac;
        URI uri;

        for (int pos = 0; pos < Packet_eHDDP.getNumHops(); pos++){
            Mac = Long.toHexString(Packet_eHDDP.getidmacdevices()[pos]);
            ...
            switch (type_device[pos]) {
                ...
                case TYPE_SDN_NO_CONFIG:
                    ...
                    if (!Nodos_SDN.contains(uri.toString()) && Nodos_SDN.size() < num_dev_sdn_file )
                    {
                        log.info("Como la ID no esta en la lista, tomamos medida {} en el instnate {} ",
                        ...
                        log.info("La ID {} ya esta en la lista {}", uri, Nodos_SDN);
                    }
                ...
                case TYPE_NO_SDN:
                    ...
                    try {
                        ...
                        checkportofdevice(dpsv, dsv, Packet_eHDDP, uri, pos);
                    } catch (Exception e) {
     ...
     */
    public boolean checkportperdevices(DeviceProviderService dpsv, DeviceService dsv, eHDDPpacket Packet_eHDDP){
        short type_device[] = Packet_eHDDP.getTypedevices();
        int ports_in[] = Packet_eHDDP.getinports(), ports_out[] = Packet_eHDDP.getoutports();
        String Nom_URI = "";
        ...
        for (int pos = 0; pos < Packet_eHDDP.getNumHops(); pos++){
            switch(type_device[pos])
            {
                ...
                case TYPE_SDN_NO_CONFIG:
                    Nom_URI = "of:"+parser_idpacket_to_iddevice(Packet_eHDDP.getidmacdevices()[pos]);
                    break;
                case TYPE_NO_SDN:
                    Nom_URI = "sw:"+Long.toHexString(Packet_eHDDP.getidmacdevices()[pos]);
                    break;
                default: /* Sensor case, only when we can connect two sensor between them*/
                    Nom_URI = Long.toHexString(Packet_eHDDP.getidmacdevices()[pos]);
                    break;
            }
            ...
            checkportofdevice(dpsv, dsv, Packet_eHDDP, URI.create(Nom_URI), pos);
        }
     ...
     * @return true Si se han creado y asignado correctamente los puerto a los dispositivos; false en caso contrario
     */
    private void checkportofdevice(DeviceProviderService dpsv, DeviceService dsv, eHDDPpacket Packet_eHDDP,
                                   URI uri, int pos){
        int num_port = 0;
        int port_in = Packet_eHDDP.getinports()[pos], port_out = Packet_eHDDP.getoutports()[pos];
        /** Matriz de puertos a insertar, máximo 2 puertos por switch y paquete recibido */
    ...
    public PortDescription CreatePortDescription(Port.Type Type, long Port, long speed) {
        return DefaultPortDescription.builder().withPortNumber(portNumber(Port)).isEnabled(true).
    ...
    public boolean removedevice(DeviceProviderService deviceProviderService, DeviceService deviceService, DeviceId deviceid){
        try{
            ...
            log.error("eHDDPAPP ERROR :AL BORRAR LOS DISPOSITIVOS");
