...
parse_oxm_entry(struct ofl_match *match, const struct oxm_field *f,
    ...
    switch (f->index) {
        ...
        case OFI_OXM_OF_EHDDP_FLAGS:
        case OFI_OXM_OF_EHDDP_OPCODE:
...
/*Modificacion UAH Discovery hybrid topologies, JAH-
static void oxm_put_64_dht(struct ofpbuf *buf, uint32_t header, uint64_t * value)
{
    oxm_put_header(buf, header);
    ofpbuf_put(buf, value, sizeof(uint64_t)*DHT_MAX_ELEMENTS);
}

static void oxm_put_32_dht(struct ofpbuf *buf, uint32_t header, uint32_t * value)
{
    oxm_put_header(buf, header);
    ofpbuf_put(buf, value, sizeof(uint32_t)*DHT_MAX_ELEMENTS);
}
Fin Modificacion UAH Discovery hybrid topologies, JAH-*/
