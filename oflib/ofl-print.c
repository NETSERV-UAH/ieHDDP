...
ofl_oxm_type_print(FILE *stream, uint32_t type){
    ...
    case OXM_OF_EHDDP_FLAGS:         {fprintf(stream, "Flags and version"); return; }
    case OXM_OF_EHDDP_OPCODE:         {fprintf(stream, "Opcode"); return; }
    case OXM_OF_EHDDP_NUM_DEVICE:	    {fprintf(stream, "Num_Devices"); return; }
    case OXM_OF_EHDDP_NUM_SEC:         {fprintf(stream, "Num Secuence"); return; }
    case OXM_OF_EHDDP_NUM_ACK:         {fprintf(stream, "Num ACK"); return; }
    case OXM_OF_EHDDP_PRE_MAC_SIZ:         {fprintf(stream, "Previous Mac Size"); return; }
    case OXM_OF_EHDDP_PRE_MAC:         {fprintf(stream, "Previous Mac"); return; }
    case OXM_OF_EHDDP_LAS_MAC:         {fprintf(stream, "LAST Mac"); return; }
    case OXM_OF_EHDDP_SRC_MAC:         {fprintf(stream, "Source Mac"); return; }
    case OXM_OF_EHDDP_TIM_BLO:         {fprintf(stream, "Block Time"); return; }
    case OXM_OF_EHDDP_CONFIG:         {fprintf(stream, "Configurations"); return; }
    case OXM_OF_EHDDP_TYPE_DEVICE:    {fprintf(stream, "Type_device"); return; }
    case OXM_OF_EHDDP_IDS:           {fprintf(stream, "Ids"); return; }
    case OXM_OF_EHDDP_IN_PORTS:       {fprintf(stream, "In_port"); return; }
    case OXM_OF_EHDDP_OUT_PORTS:      {fprintf(stream, "Out_port"); return; } 
